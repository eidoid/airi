...
const { isRunning, isThinking, lastTriggeredAt, lastError, lastOutcome } = storeToRefs(proactiveStore)
...
const lastOutcomeLabel = computed(() => {
  if (!lastOutcome.value?.skippedReason)
    return lastErrorLabel.value
  return t('settings.pages.modules.proactive-speech.sections.section.runtime.last-skipped.at', {
    reason: lastOutcome.value.skippedReason,
  })
})

function triggerNow() {
  void proactiveStore.trigger({ manual: true })
}
</script>
...
<template>
  <div bg="neutral-50 dark:[rgba(0,0,0,0.3)]" rounded-xl p-4 flex="~ col gap-6">
    ...
    <section flex="~ col gap-4">
      ...
      />
      <FieldCheckbox
        v-model="showStageTimer"
        :label="t('settings.pages.modules.proactive-speech.sections.section.schedule.show-stage-timer.title')"
        :description="t('settings.pages.modules.proactive-speech.sections.section.schedule.show-stage-timer.description')"
      />
      <FieldRange
